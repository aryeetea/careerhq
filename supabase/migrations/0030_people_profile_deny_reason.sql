-- Give get_people_profile a reason code for why a profile isn't visible,
-- so the client can show a specific, honest message instead of one
-- catch-all "not available" state. The reason itself is deliberately
-- coarse — "not_found" vs "blocked" vs "no_access" — it never reveals
-- more about *why* access is denied than a normal user should be able to
-- infer (e.g. blocked users still just see "not available").
drop function if exists get_people_profile(uuid, text);

create function get_people_profile(p_user_id uuid, p_preview text default null)
returns table (
  user_id uuid,
  username text,
  display_name text,
...
begin
  ...
  where user_id = p_user_id and week_start = wk;
  ...
  select
    ...
    case when allow_certification then cert.progress_percentage else null end,
    case
