...
function describeEvent(e: ActivityEvent): string {
  switch (e.type) {
    ...
    case "reaction_received": {
      ...
      return meta ? `Someone sent you "${meta.label}" ${meta.emoji}` : "Someone sent you encouragement";
    }
    case "group_invite_received":
      return "You've been invited to a group";
    default:
      return "New activity";
  }
}
            ...
            notifications.map((n) => (
              ...
              >
                <span className="flex w-full items-center gap-2 text-sm">
                  ...
                  <span className={n.read_at ? "text-muted-foreground" : "font-medium"}>{describeEvent(n)}</span>
                </span>
