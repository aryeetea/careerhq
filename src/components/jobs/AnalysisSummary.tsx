    ...
    <div className="rounded-xl border border-border/60 bg-card/50 p-3">
      ...
      {items.length === 0 ? (
        ...
        <ul className="mt-2 grid gap-1.5 text-sm">
          {items.map((item) => (
            <li key={item} className="rounded-lg bg-secondary/50 px-2.5 py-2">
              {item}
            </li>
          ))}
        </ul>
  ...
  return (
    <div className="grid gap-3">
        ...
        <CardContent className="p-4">
          <div className="flex flex-wrap items-start justify-between gap-3">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <Badge variant="outline">Priority {analysis.priority}</Badge>
            </div>
          </div>
          {recommended && (
      ...
      <div className="grid gap-3 lg:grid-cols-2">
        <ListBlock title="Matching strengths" items={analysis.matching_strengths} empty="No strong matches were identified from the available resume evidence." />
        ...
        <ListBlock title="Missing required qualifications" items={analysis.missing_required_qualifications} empty="No required gaps were identified." />
        <ListBlock title="Missing preferred qualifications" items={analysis.missing_preferred_qualifications} empty="No preferred gaps were identified." />
      </div>
